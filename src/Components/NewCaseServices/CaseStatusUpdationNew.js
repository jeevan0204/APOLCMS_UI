...
function CaseStatusUpdationNew() {
    ...
    console.log("--------para----", parawiseFileExist1)
    const userValidations = Yup.object().shape({
        ...
        counterFileCopy: Yup.array()
            .when(['ecourtsCaseStatus', 'counterFiled'], {
                is: (status, submitted) => status === 'Pending' && submitted === 'Yes' && !counterFileExist1,
                then: (schema) =>
                    schema
                        .of(
                            Yup.mixed()
    ...
    const formIk = useFormik({
        ...
        onSubmit: (values) => {
            ...
            //alert("hlo");
            values.fileCino = ackNo;
            //console.log("values---------", values)
            if (caseDetails?.STATUSUPDATEBTN !== "") {
                const url = config.url.local_URL + "updateCaseDetailsNew";
                console.log("url------", url)
                CommonAxiosPost(url, values).then((res) => {
                    if (res?.data?.status === true) {
                        successAlert2(res?.data?.sdesc).then(() => {
                            ...
                            if (
                                ...
                                console.log("edit------", record[0]?.OLCMSCASEDATA[0].pwr_submitted_date)
    ...
    function GetCaseDetails(ackNo) {
        if (!ackNo?.trim()) {
            alert("Acknowledgment number is missing!");
            return;
        }

        resetCaseDetailsState();

        const url = config.url.local_URL + "caseStatusUpdateNew?ackNo=" + ackNo;
        ...
        CommonAxiosGet(url).then((res) => {
            ...
            if (apiResponse?.status === true) {
                ...
                setHEADING(caseData.HEADING);
                if (caseData.OLCMSCASEDATA.length > 0 && caseData.OLCMSCASEDATA[0]?.ecourts_case_status !== undefined) {

                    console.log("p--------" + JSON.stringify(caseData.OLCMSCASEDATA[0].ecourts_case_status));

                    const SetCaseDeailsData = res?.data?.data?.OLCMSCASEDATA[0];

                    formIk.setFieldValue('ecourtsCaseStatus', caseData.OLCMSCASEDATA[0].ecourts_case_status || "");

                    formIk.setFieldValue('parawiseRemarksSubmitted', caseData.OLCMSCASEDATA[0].pwr_uploaded || "");
                    formIk.setFieldValue('parawiseRemarksDt', caseData.OLCMSCASEDATA[0].pwr_submitted_date || "");
                    formIk.setFieldValue('counterFiled', caseData.OLCMSCASEDATA[0].counter_filed || "");
                    formIk.setFieldValue('counterFiledDt', caseData.OLCMSCASEDATA[0].counter_filed_date || "");
                    formIk.setFieldValue('judgementOrder', caseData.OLCMSCASEDATA[0].judgement_order || "");
                    formIk.setFieldValue('actionTakenOrder', caseData.OLCMSCASEDATA[0].action_taken_order || "");
                    formIk.setFieldValue('appealFiled', caseData.OLCMSCASEDATA[0].appeal_filed || "");
                    formIk.setFieldValue('appealFileCopy', caseData.OLCMSCASEDATA[0].appeal_filed_copy || "");
                    formIk.setFieldValue('appealFiledDt', caseData.OLCMSCASEDATA[0].appeal_filed_date || "");
                    formIk.setFieldValue('actionToPerform', caseData.OLCMSCASEDATA[0].action_to_perfom || "");
                    formIk.setFieldValue('remarks', caseData.OLCMSCASEDATA[0].remarks || "");
    ...
    function sendbackFn() {
        ...
        const url = config.url.local_URL + "sendBackCaseDetailsNew";
        const payload = { fileCino: ackNo };
    ...
    function forwardCase() {
        ...
        alert(ackNo + "-------")
        const url = config.url.local_URL + "forwardCaseDetailsNew";
        const payload = { fileCino: ackNo };
    ...
    function forwardCase2GP(gpCode) {
        ...
        const url = config.url.local_URL + "forwardCaseDetails2GPNew";
        const payload = { fileCino: ackNo, gpCode: gpCode };
                            ...
                            <table className="table table-bordered table-striped" style={{ width: "100%" }}>
                                ...
                                <tbody style={{ textAlign: "left" }}>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <i style={{ textAlign: "justify" }}>
                                                <b>Reported By :</b>&nbsp;
                                                {caseDetails.USERSLIST[0]?.inserted_by}
                                            </i>
                                        ...
                                        <td>
                                            <b>Registration No :</b>&nbsp;
                                            {caseDetails.USERSLIST[0]?.ack_no}
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <b>Case Full Name :</b>&nbsp;
                                            {caseDetails.USERSLIST[0]?.case_full_name}
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                            <i style={{ textAlign: "justify", color: "red" }}>
                                                <b>Generated Date :</b>&nbsp;
                                                {caseDetails.USERSLIST[0]?.generated_date}
                                            </i>
                                        </td>
                                        <td>
                                            <b>Main Case No :</b>&nbsp;
                                        ...
                                        <td>
                                            ...
                                            {caseDetails.USERSLIST[0]?.dept_descs}
                                        </td>
                                    </tr>
                            ...
                            <div className="border px-13 pb-13 mb-4 pt-1">
                                    ...
                                    <bst.Col xs={4} sm={4} md={4} lg={3} xl={3} xxl={3}>
                                        ...
                                        <Field as="select" className='form-control mt-3' name="ecourtsCaseStatus"
                                            onChange={(e) => {
                                                ...
                                                if (value === "Closed" || value === "Private") {
                                                    formIk.setFieldValue("parawiseRemarksSubmitted", "");
                                                    formIk.setFieldValue("counterFiled", "");
                                                    // formIk.setFieldValue("parawiseRemarksCopy", []);
                                                }
                                            }}>
                                        ...
                                        <bst.Col xs={4} sm={4} md={4} lg={3} xl={3} xxl={3}>
                                            ...
                                            <Field as="select" className='form-control mt-3' name="parawiseRemarksSubmitted"
                                                onChange={(e) => {
                                                    const value = e.target.value;
                                                    if (value === "No") {
                                                        formIk.setFieldValue("parawiseRemarksDt", "");
                                                        formIk.setFieldValue("parawiseRemarksCopy", []);
                                                    } else if (value === "Yes" && (!formIk.values.parawiseRemarksCopy || formIk.values.parawiseRemarksCopy.length === 0)) {
                                                        formIk.setFieldValue("parawiseRemarksCopy", [null]);
                                                    }
                                                }} >
                                                <option value="0">--SELECT--</option>
                                                                            ...
                                                                            <div className="d-flex align-items-center gap-2">
                                                                                <input
                                                                                    ...
                                                                                    onChange={(e) => {
                                                                                        ...
                                                                                        let fileVal = "parawiseRemarks_" + ackNo + "_" + index;
                                        ...
                                        <bst.Col xs={4} sm={4} md={4} lg={3} xl={3} xxl={3}>
                                            ...
                                            <Field as="select" className='form-control' name="counterFiled"
                                                onChange={(e) => {
                                                    ...
                                                    if (value === "No") {
                                                        formIk.setFieldValue("counterFiledDt", "");
                                                        formIk.setFieldValue("counterFileCopy", []);
                                                    } else if (value === "Yes" && (!formIk.values.counterFileCopy || formIk.values.counterFileCopy.length === 0)) {
                                                        formIk.setFieldValue("counterFileCopy", [null]);
                                                    }
                                                }}
                                            >
                                                <option value="0">--SELECT--</option>
                                                                        ...
                                                                        <bst.Col xs={6} sm={6} md={6} lg={6} xl={6} xxl={6}>
                                                                            ...
                                                                            <div className="d-flex align-items-center gap-2">
                                                                                <input
                                                                                    ...
                                                                                    onChange={(e) => {
                                                                                        const file = e.target.files[0];
                                                                                        let fileVal = "counters_" + ackNo + "_" + index;
                                                                                        let filename = fileVal;
                                ...
                                {formIk?.values?.ecourtsCaseStatus === "Closed" && (<>
                                    <bst.Row className="px-4 pt-4">
                                                ...
                                                {({ field, form }) => (
                                                    <input
                                                        ...
                                                        onChange={(e) => {
                                                            const file = e.target.files[0];
                                                            if (file) {
                                                                let fileVal = "judgementorder_" + ackNo;
                                                                let filename = fileVal;
                                                ...
                                                {({ field, form }) => (
                                                    <input
                                                        ...
                                                        onChange={(e) => {
                                                            const file = e.target.files[0];
                                                            if (file) {
                                                                let fileVal = "actionorder_" + ackNo;
                                                                let filename = fileVal;
                                        ...
                                        <bst.Col xs={6} sm={6} md={6} lg={4} xl={4} xxl={4}>
                                            <label className="mb-3"> Appeal Filed  <span style={{ color: 'red' }}>*</span></label>
                                            <Field as="select" className='form-control' name="appealFiled"
                                                onChange={(e) => {
                                                    const value = e.target.value;
                                                    if (value === "No") {
                                                        formIk.setFieldValue("appealFiledDt", "");
                                                        formIk.setFieldValue("appealFileCopy", []);
                                                    }
                                                }} >
                                                <option value="0">--SELECT--</option>
                                                <option value="No">No</option>
...
export default CaseStatusUpdationNew