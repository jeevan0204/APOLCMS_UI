...
function PPBarcodeGenerationLegacy() {
        ...
        onSubmit: (values) => {
            ...
            CommonAxiosGetPost(url, values).then((res) => {
                ...
                if (res?.data?.status === true) {
                    ...
                    navigate("/PPOAcknowledgementForm", {
                        state: {
            ...
            <div style={{ width: "95%", paddingTop: "100px" }}>
                {casesList?.length > 0 ? (
                    <CommonReactTable data={casesList} columns={columnsList} showFooter={"true"}
                        filename="pp case details REPORT" headerName="Case Details" />
                ) : (errmsg && (
...
export default PPBarcodeGenerationLegacy