...
function YearWisePendingCasesReport() {
    const [PendingCasesReport, setPendingCasesReport] = useState([]);
    ...
    function GetYearWiseReportList() {
        const section_code = formIk?.values?.section_code;
        const FromDate = formIk?.values?.FromDate;
        const ToDate = formIk?.values?.ToDate;

        let url = config.url.local_URL + "YearWisePendingCasesReport?fromDate=" + FromDate + "&toDate=" + ToDate +
            "&section_code=" + section_code;
        CommonAxiosGet(url).then((res) => {
            if (res?.data?.status === true) {
                setDrillCount(0);
                setPendingCasesReport(res.data.CASESLIST);
                setSection(res?.data?.section);

            } else {
                setPendingCasesReport([]);
            }
        }).catch((error) => {
    ...
    const columns = [
        ...
        {
            Header: 'Division Details', accessor: "",
            Cell: ({ row }) => {
        ...
        {
            Header: 'Upto Year(Total)', accessor: "",
            isClickable: true,
            Cell: ({ row }) => (
        ...
        {
            Header: 'Counter Filed upto Year', accessor: "",
            isClickable: true,
            Cell: ({ row }) => (
                <div style={{ cursor: "pointer", textDecoration: "underline", color: "blue", textAlign: "left", }}
                    onClick={() => {
                        getCasesList(row.original.email, section, 'COUNTER', 'DATE');
                    }}
                >
        ...
        {
            Header: 'Net Balance', accessor: "",
            isClickable: true,
            Cell: ({ row }) => (
                <div style={{ cursor: "pointer", textDecoration: "underline", color: "blue", textAlign: "left", }}
                    onClick={() => {
                        getCasesList(row.original.email, section, 'NETBAL', 'DATE');
                    }}
                >
                    {row.original.net_balance}
                </div>
            ),
            Footer: "net_balance"
        ...
        {
            Header: 'Amount Involved(Rs in Lakhs)', accessor: "",
            Cell: () => '0.00',
    ...
    function getCasesList(email, section, ids, date) {
        let url = config.url.local_URL + "CasesListSectionOfficerYear?section_code=" + section + "&emailId=" + email + "&ids=" + ids + "&date=" + date;
        CommonAxiosGet(url).then((res) => {
            setDrillCount(1);
    ...
    console.log("section---" + section, errormsg)

    return (
        <>

            <CommonFormHeader heading="Year Wise Cases Report" />
            <bst.Container className='outer-page-content-container'>
                ...
                {showDrillCount === 0 && (<>
                    {PendingCasesReport?.length > 0 ? (
                        <CommonReactTable data={PendingCasesReport} columns={columns} showFooter={"true"}
                            filename="Year wise Pending Cases Report" headerName="" />
                    ) : ((
                    ...
                    {section === 'L' && casesList?.length > 0 ? (
                        <div style={{ width: "98%", marginTop: "5%" }}>
                            <CommonReactTable data={casesList} columns={columnlist} showFooter={"true"}
                                filename="Year Wise Pending legacy Cases List" isBack={true} isBackFunction={isBackFunction} />
                        </div>
                    ) : (
                        errormsg && (<center><b style={{ color: "red" }}>*********No Data Found*********</b></center>))}</>)}

                {showDrillCount === 1 && (<>
                    {section === 'N' && casesList?.length > 0 ? (
                        <div style={{ width: "98%", marginTop: "5%" }}>
                            <CommonReactTable data={casesList} columns={columnsListNew} showFooter={"true"}
                                filename="Year Wise Pending New Cases List" isBack={true} isBackFunction={isBackFunction} />
                        </div>
                    ) : (section === 'N' && (<center><b style={{ color: "red" }}>*********No Data Found*********</b></center>))}</>)}
...
export default YearWisePendingCasesReport