...
function RegisterMLO() {
    ...
    const formIk = useFormik({
        ...
        onSubmit: (values) => {
            console.log(values);
            const url = config.url.local_URL + "saveEmployeeDetailsMLO";
    ...
    function GetDesignationList() {
        let url = config.url.local_URL + "DesignationListMLO";
        CommonAxiosGet(url).then((res) => {
    ...
    function GetData() {
        let url = config.url.local_URL + "registerMLO";
        CommonAxiosGet(url).then((res) => {
            if (res.data.status === true) {
    ...
    const columnsList = [
        ...
        },
        {
            ...
            Cell: ({ row }) => (
                <button
                    onClick={() => sendSMS(row.original.aadharno, row.original.emailid, 'MLO')}
                    style={{
    ...
    return (
        <>
            <CommonFormHeader heading="Mid Level Officer (Legal) Registration" />
            <bst.Container className='outer-page-content-container'>
                <bst.Row className="pt-2 pt-2 ">
                    <FormikProvider value={formIk}>
                        <Form onSubmit={formIk?.handleSubmit} onChange={formIk?.handleChange} >
                            {console.log("-------->>", ListData.length, saveAction)}
                            {ListData.length === 0 && saveAction === "INSERT" && (<>
